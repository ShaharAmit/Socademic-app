...
public class gAdapter extends RecyclerView.Adapter<gAdapter.MyViewHolder> {
    ...
    private ArrayList<gItem> mDataset;
    private int textSize = 18;

    // Provide a suitable constructor (depends on the kind of dataset)
    public gAdapter(ArrayList<gItem> myDataset,
    ...
    public void onBindViewHolder(MyViewHolder holder, int position) {
        ...
        gItem arrayItem = mDataset.get(position);
        holder.mTextView.setText(arrayItem.getLabel());
        holder.mTextView.setTag(arrayItem); // a handy way to pass any data to a view
        holder.mTextView.setOnClickListener(listener);

        holder.mTextView.setTextSize(TypedValue.COMPLEX_UNIT_SP, textSize);
    }
